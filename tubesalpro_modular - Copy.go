...
const NMAX = 100         // Kapasitas maksimum array daftarKegiatan
const MAX_HARI_UNIK = 10 // Kapasitas maksimum untuk statistik harian unik

var daftarKegiatan [NMAX]Kegiatan // Array untuk menyimpan daftar kegiatan
var jumlahKegiatanSekarang = 0    // Menyimpan jumlah kegiatan yang ada saat ini
...
func main() {
	...
	for state {
		...
		fmt.Scan(&pilihan) // No error check, no buffer clearing
...
func updateKegiatan() {
	...
	if indeks != -1 {
		...
		fmt.Scan(&namaBaru)
		if namaBaru != "" { // Prevents blanking if fmt.Scan somehow allows it (unlikely)
		...
		if jumlahPesertaBaru < 0 {
			...
			return // Return to avoid setting invalid value
		}
...
func cariKegiatan() {
	...
	if pilihanCari == "nama" { // Case-sensitive
		var keyword string
	...
	for i := 0; i < jumlahKegiatanSekarang; i++ {
		...
		if !ditemukan {
			if jumlahHariUnik < MAX_HARI_UNIK {
				...
				// Optionally, notify that MAX_HARI_UNIK capacity is reached
				// For simplicity, we're not adding that notification here.
			}
		}
	}
...
// Prosedur: Mengurutkan jumlah peserta dengan Insertion Sort (dari kecil ke besar)
func urutkanPesertaAsc() {
	var i, j int
	var temp Kegiatan
	for i = 1; i < jumlahKegiatanSekarang; i++ {
		temp = daftarKegiatan[i]
	...
	fmt.Println("Data berhasil diurutkan berdasarkan jumlah peserta (ascending).")
}

// Prosedur: Mengurutkan jumlah peserta dengan Selection Sort (dari besar ke kecil)
func urutkanPesertaDesc() {
	var i, j, maxIdx int
	for i = 0; i < jumlahKegiatanSekarang-1; i++ {
		maxIdx = i
		for j = i + 1; j < jumlahKegiatanSekarang; j++ {
	...
	fmt.Println("Data berhasil diurutkan berdasarkan jumlah peserta (descending).")
}

// Fungsi: Mengecek apakah hari kegiatan tabrakan (binary search)
func adaTabrakanHari(hari string) bool {
	var kiri, kanan, tengah int
	kiri = 0
	kanan = jumlahKegiatanSekarang - 1
	for kiri <= kanan {
		tengah = (kiri + kanan) / 2
		if daftarKegiatan[tengah].Hari == hari {
			return true
		} else if daftarKegiatan[tengah].Hari < hari {
			kiri = tengah + 1
		} else {
			kanan = tengah - 1
		}
...
// Prosedur: Mengecek tabrakan setelah data diurutkan berdasarkan hari (bubble sort)
func cekTabrakanHari() {
	if jumlahKegiatanSekarang == 0 {
		fmt.Println("Belum ada kegiatan yang terdaftar.")
		return
	...
	var hari string
	fmt.Print("Masukkan hari yang ingin dicek: ")
	fmt.Scan(&hari)
	...
	if adaTabrakanHari(hari) {
		fmt.Println("Terdapat tabrakan kegiatan pada hari tersebut.")
	} else {
		fmt.Println("Tidak ada tabrakan kegiatan pada hari tersebut.")
	}
}

// Prosedur Rekursif: Menampilkan seluruh data kegiatan
func tampilkanRekursif(index int) {
	if index >= jumlahKegiatanSekarang {
		return
	}
	fmt.Printf("ID: %d | Nama: %s | Hari: %s | Jumlah Peserta: %d\n",
		daftarKegiatan[index].ID, daftarKegiatan[index].NamaKegiatan,
		daftarKegiatan[index].Hari, daftarKegiatan[index].JumlahPeserta)
	tampilkanRekursif(index + 1)
}
...
func tampilkanSemuaRekursif() {
	...
	fmt.Println("\nMenampilkan seluruh kegiatan secara rekursif:")
	tampilkanRekursif(0)
}
